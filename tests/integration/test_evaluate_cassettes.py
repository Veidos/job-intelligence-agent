...
class TestCoherenceCheck:
    """Tests para coherence_check() con cassettes."""

    def test_coherence_confirma_cuando_hr_yes_pero_score_bajo(
        self, sample_offer_no_exp, sample_perfil_text
    ):
        ...
        hr = {
            "apply_signal": "maybe",
            "verdict": "Podría considerarse esta oferta",
        }
        technical = {
            "skills_hard_match": 10,
            "experience_match": 5,
            "education_match": 3,
            "location_match": 2,
        }
        ...
        coherence_confirm = CASSETTES["coherence_confirms_apply"]

        with patch("src.pipeline.evaluate.ollama_call") as mock:
            mock.return_value = coherence_confirm
            revised_score, note = coherence_check(
                sample_offer_no_exp, sample_perfil_text, technical, hr, raw_score
            )

        assert revised_score > raw_score
        assert note is not None
        assert "Score ajustado" in note

    def test_coherence_rechaza_cuando_no_confirma(
        self, sample_offer_senior, sample_perfil_text
    ):
        from src.pipeline.evaluate import coherence_check

        hr = {
            "apply_signal": "maybe",
            "verdict": "Quizás merezca la pena",
        }
        technical = {
            "skills_hard_match": 8,
            "experience_match": 5,
        ...
        coherence_reject = CASSETTES["coherence_rejects"]

        with patch("src.pipeline.evaluate.ollama_call") as mock:
            mock.return_value = coherence_reject
            revised_score, note = coherence_check(
                sample_offer_senior, sample_perfil_text, technical, hr, raw_score
            )

        assert revised_score == raw_score
        assert note is not None
        assert "HR optimista" in note or "técnico no confirma" in note

    def test_no_ajusta_cuando_apply_signal_no(self, sample_offer, sample_perfil_text):
        from src.pipeline.evaluate import coherence_check

        hr = {"apply_signal": "no", "verdict": "No recomienda"}
    ...
    def test_no_ajusta_cuando_score_mayor_35(self, sample_offer, sample_perfil_text):
        from src.pipeline.evaluate import coherence_check
