...
3. `compute_experience_score()` — Python computes `F_exp = years_match` (gap is qualitative context for HR — see ADR-013).
...
- **Employment gap was moved to qualitative HR context** (ADR-013). No numeric penalty in F_exp.
